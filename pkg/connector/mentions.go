...
// (pkg/msgconv/gchatfmt) and matrixfmt.Parse (pkg/msgconv/matrixfmt) leave
// to their callers, plus content.Mentions ("m.mentions")
...
// user's own MXID when the gaia id belongs to one of this bridge's own
// logged-in accounts, otherwise a ghost's MXID computed by a pure,
// deterministic formula (no DB lookup, no requirement that a ghost row
// already exist -- a gaia id mentioned for the first time still resolves).
// gchatMentionResolver below implements this via bridgev2's equivalents:
// Bridge.GetCachedUserLoginByID for the logged-in-account lookup, and
// Bridge.Matrix.GhostIntent(id).GetMXID() (the same formula bridgev2 itself
// uses to name a ghost's Matrix user, ghost.go:58 in mautrix-go -- also
// pure, no I/O) for the ghost MXID. A room-member-state-store lookup to
// override the mention text with the member's displayname (only for the
// double-puppet branch) is deliberately NOT done here: gchatfmt's
...
// gchatfmt's renderMention falls back to entity_text for, matching the
// unconditional entity_text default for that branch. For the double-puppet
// branch specifically, gchatMentionResolver substitutes login.RemoteName
// (the double-puppeted account's network-side display name) instead of a
// room-member displayname -- a deliberately different, DB-free source for a
// similar effect; see gchatMentionResolver's own doc comment.
//
...
// for GC->Matrix; handlematrix.go for Matrix->GC).
import (
	"context"
	"errors"
...
//     accounts (double puppet), the mention resolves to that account's real
//     Matrix user (login.UserMXID), with login.RemoteName as the display
//     name.
//  2. Otherwise it resolves to a ghost, via ghostMXID -- with name left ""
//     (see the package doc comment on why the display-name override isn't
//     ported).
//
...
// ("m.mentions") block that fixes B2/gap G4:
...
// (_reference/meta/pkg/msgconv/textfmt/from-matrix.go):
//
...
// bridge (handlematrix.go call site). nil-safe like
